...
import { ShieldCheck, Plus, Copy, LogOut, Key, Server } from 'lucide-react';
...
export const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  // SWR Key depends on token to prevent premature fetch
  const { data: vpsList, mutate } = useSWR(token ? '/vps/list' : null, fetcher);
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <aside className="w-64 border-r bg-card flex flex-col hidden md:flex shrink-0">
        ...
        <div className="p-6 border-b shrink-0">
          ...
          <Button onClick={() => setIsModalOpen(true)} className="w-full justify-start gap-2 shadow-emerald-500/20 shadow-lg" variant="default">
            ...
            return (
              <Link href={`/dashboard/${vps._id}`} key={vps._id}>
                <Button
                  ...
                  className={cn("w-full justify-start gap-2 mb-1", isActive && "bg-secondary/80 font-semibold border border-border/50")}
                >
                  <div className={`h-2 w-2 rounded-full shadow-[0_0_8px] ${vps.status === 'online' ? 'bg-emerald-500 shadow-emerald-500/50' : 'bg-destructive shadow-destructive/50'}`} />
                  <span className="truncate">{vps.name}</span>
                </Button>
              </Link>
            )
          })}
          {vpsList?.length === 0 && <div className="px-2 text-sm text-muted-foreground text-center py-4 border border-dashed rounded-md">No servers found.<br />Connect one above.</div>}
        </div>
        ...
        <div className="p-4 border-t bg-card/50 shrink-0">
          <div className="flex items-center gap-3 mb-4 p-2 rounded-lg bg-secondary/20">
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-sm font-medium truncate">{user.displayName || 'Administrator'}</span>
          ...
          <Button onClick={logout} variant="outline" size="sm" className="w-full gap-2 hover:bg-destructive hover:text-destructive-foreground hover:border-destructive transition-colors">

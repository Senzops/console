...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.code === 'auth/invalid-credential' ? 'Invalid email or password.' : 'Failed to sign in. Please try again.';
      setError(msg);
    } finally {
      setIsLoading(false);
    }
    ...
    <AuthLayout>
      <Card className="w-full max-w-md border-border/50 shadow-2xl">
        ...
        <CardContent className="space-y-4">
          ...
          <Button variant="outline" className="w-full relative" onClick={async () => { await loginGoogle(); if (redirect) router.replace(redirect); }} disabled={isLoading}>
